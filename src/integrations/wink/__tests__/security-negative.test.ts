...
describe("certified bridge security negatives — BlockBlast", () => {
  ...
  it("fails closed on config drift (wrong bridge version)", async () => {
    const harness = await createHarness({
...
describe("BlockBlast iframe-only documentation boundary", () => {
  ...
  it("adapter source contains no forbidden authority patterns", () => {
    const adapterSource = fs.readFileSync(
      path.join(ROOT, "src/integrations/wink/client.ts"),
      "utf8",
    );
    expect(adapterSource).not.toMatch(/postMessage\s*\(/);
    expect(adapterSource).not.toMatch(/fetch\s*\(/);
    expect(adapterSource).not.toMatch(/XMLHttpRequest/);
    expect(adapterSource).not.toMatch(/localStorage/);
    expect(adapterSource).not.toMatch(/sessionStorage/);
    expect(adapterSource).not.toMatch(/document\.cookie/);
    expect(adapterSource).not.toMatch(/accessToken/);
    expect(adapterSource).not.toMatch(/\/api\/v1/);
  });
});
